...
**What this endpoint expresses:** `GET /passenger-categories` describes the Fare Provider's own categorisation scheme — how that specific provider defines and labels its passenger groups. A category may be based on age bracket (e.g. child, adult, senior), possession of a reduction card, or a social/legal right (e.g. unemployed, disabled). This is a provider-specific taxonomy, not a standardised enumeration of universal passenger types. A more modern approach is to pass eligibility criteria directly in the offer request (date of birth, card reference, rights entitlement, chipcard content), which avoids the need for the client to first resolve which local category applies. However, many fare systems still operate on the category-based model, making this endpoint necessary for interoperability with those systems.
